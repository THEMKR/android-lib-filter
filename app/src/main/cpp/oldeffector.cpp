...
float getMultiplyColor(float srcColor, float overlayColor) {
    return 255 * getFloatColorValue(srcColor) * getFloatColorValue(overlayColor);
}

float getOverlayColor(float srcColor, float overlayColor) {
    if (srcColor < 128) {
...
Java_com_mkrworld_libfilter_effect_BaseEffect_getConventionalEffectedBitmap(JNIEnv *jEnv,
                                                                            jobject obj,
    ...
    for (y = 0; y <= bitmapInfoOrignal.height - 1; y++) {
        ...
        for (x = 0; x < bitmapInfoOrignal.width - 1; x++) {
            ...
            if (!(y == 0 || y == bitmapInfoOrignal.height - 1 || x == 0 ||
                ...
                R = (R * multiplier + offSet);
                G = (G * multiplier + offSet);
                B = (B * multiplier + offSet);
            }
...
extern "C" JNIEXPORT void JNICALL
Java_com_mkrworld_libfilter_effect_BaseEffect_getColorEffectedBitmap(JNIEnv *jEnv, jobject obj,
                                                                     jobject bitmap,
                                                                     jfloat multiplier,
                                                                     jfloatArray effectArray) {
    AndroidBitmapInfo bitmapInfoOrignal;
    ...
    if ((ret = AndroidBitmap_getInfo(jEnv, bitmap, &bitmapInfoOrignal)) < 0) {
        LOGE("AndroidBitmap_getInfo() bitmapGrayscale failed ! error=%d", ret);
        return;
    }
    if (bitmapInfoOrignal.format != ANDROID_BITMAP_FORMAT_RGBA_8888) {
    ...
    if ((ret = AndroidBitmap_lockPixels(jEnv, bitmap, &pixelsOrignal)) < 0) {
        LOGE("AndroidBitmap_lockPixels() failed ! error=%d", ret);
    }
    jfloat *array = jEnv->GetFloatArrayElements(effectArray, 0);
    float colorMatrix[4][5] = {{array[0],  array[1],  array[2],  array[3],  array[4]},
    ...
    for (y = 0; y < bitmapInfoOrignal.height; ++y) {
        ...
        for (x = 0; x < bitmapInfoOrignal.width; ++x) {
            ...
            orignalLine[x].red = getColorValue(newRed + colorMatrix[0][4]);
            orignalLine[x].green = getColorValue(newGreen + colorMatrix[1][4]);
            orignalLine[x].blue = getColorValue(newBlue + colorMatrix[2][4]);
            orignalLine[x].alpha = getColorValue(alpha);
        }
        pixelsOrignal = (char *) pixelsOrignal + bitmapInfoOrignal.stride;
    }
    LOGE("5");
...
extern "C" JNIEXPORT void JNICALL
Java_com_mkrworld_libfilter_effect_BaseEffect_getMultiplyBitmap(JNIEnv *jEnv, jobject obj,
    ...
    for (y = 0; y < bitmapInfoOrignal.height; ++y) {
        ...
        for (x = 0; x < bitmapInfoOrignal.width; ++x) {
            ...
            effectedLine[x].red = getColorValue(getMultiplyColor(red, redOverlay) * multiplier);
            effectedLine[x].green = getColorValue(
                    getMultiplyColor(green, greenOverlay) * multiplier);
            effectedLine[x].blue = getColorValue(getMultiplyColor(blue, blueOverlay) * multiplier);
...
extern "C" JNIEXPORT void JNICALL
Java_com_mkrworld_libfilter_effect_BaseEffect_getOverLayBitmap(JNIEnv *jEnv, jobject obj,
                                                               jobject bitmapOrignal,
                                                               jobject bitmapOverlay,
                                                               jobject bitmapEffecte,
                                                               jfloat multiplier) {
    ...
    for (y = 0; y < bitmapInfoOrignal.height; ++y) {
        ...
        for (x = 0; x < bitmapInfoOrignal.width; ++x) {
            ...
            effectedLine[x].red = getColorValue(getOverlayColor(red, redOverlay) * multiplier);
            effectedLine[x].green = getColorValue(
                    getOverlayColor(green, greenOverlay) * multiplier);
            effectedLine[x].blue = getColorValue(getOverlayColor(blue, blueOverlay) * multiplier);
...
Java_com_mkrworld_libfilter_effect_BaseEffect_applyColorEffect(JNIEnv *jEnv, jobject obj,
    ...
    jint pixelCount = jEnv->GetArrayLength(bitmapPixels);
    jintArray outPutResult = jEnv->NewIntArray(pixelCount);
    jint fill[pixelCount];
    jint *pixelArray = jEnv->GetIntArrayElements(bitmapPixels, 0);
    for (jint y = 0; y < height; ++y) {
        for (jint x = 0; x < width; ++x) {
            jint pixel = pixelArray[y * width + x];
            ...
            jint newRed = (int) ((float) ((colorMatrix[0][0] * red) + (colorMatrix[0][1] * green) +
                                          (colorMatrix[0][2] * blue)) * multiplier);
            jint newGreen = (int) (
                    (float) ((colorMatrix[1][0] * red) + (colorMatrix[1][1] * green) +
                             (colorMatrix[1][2] * blue)) * multiplier);
            jint newBlue = (int) ((float) ((colorMatrix[2][0] * red) + (colorMatrix[2][1] * green) +
                                           (colorMatrix[2][2] * blue)) * multiplier);

            fill[y * width + x] =
                    getJavaColorValue(alpha) | getJavaColorValue(newRed + colorMatrix[0][4]) |
                    getJavaColorValue(newGreen + colorMatrix[1][4]) |
                    getJavaColorValue(newBlue + colorMatrix[2][4]);
    ...
    jEnv->SetIntArrayRegion(outPutResult, 0, pixelCount, fill);
    jEnv->SetIntArrayRegion(bitmapPixels, 0, pixelCount, pixelArray);
    jEnv->ReleaseIntArrayElements(bitmapPixels, pixelArray, 0);
    return outPutResult;
}
